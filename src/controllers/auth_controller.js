    ...
    try {
      ...
      if (token) {
        ...
        return res.status(200).json({
          message:
            "Logout successful. You have been logged out from the current device.",
        });
      }
      // If no token is provided, send a 401 error response
      res.status(401).json({
        error: "Unauthorized Request. No authorization header found.",
      });
    } catch (error) {
      console.error(error);
  ...
  logoutAll = async (req, res) => {
    try {
      ...
      if (token) {
        ...
        return res.status(200).json({
          message:
            "Logout successful. You have been logged out from all devices.",
        });
      }
      // If no token is provided, send a 401 error response
      res
        .status(401)
        .json({ error: "Bad Request. No authorization headers found." });
    } catch (error) {
      console.error(error);
      // If an error occurs during logout from all devices, throw a custom application error
      throw new ApplicationError(
        500,
        "Failed to logout from all devices. Please try again later."
      );
    }
  };
