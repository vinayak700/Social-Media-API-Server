...
export default class LikeController {
        ...
        try {
            ...
            if (type === 'post') {
                ...
                if (like) {
                    ...
                    await Post.updateOne({ _id: new ObjectId(id) }, { $inc: { likes: -1 } });
                } else {
                ...
                return res.status(200).json('Post like toggled');
            }
            ...
            if (type === 'comment') {
                ...
                return res.status(200).json('Comment like toggled');
            }

            // If type is neither post nor comment, return a 404 response
            return res.status(404).json('No like found!');
        } catch (error) {
            console.log(error);
    ...
    getLikes = async (req, res, next) => {
        ...
        try {
            ...
            if (type === 'comment') {
                const likes = await Like.find({ commentId: new ObjectId(id) }).populate('userId');
